...
static inline unsigned long round_hint_to_min_huge(unsigned long hint)
{
	hint &= HPAGE_PMD_MASK;
	if (((void *)hint != NULL) &&
	    (hint < mmap_min_addr))
		return ALIGN(mmap_min_addr, HPAGE_PMD_SIZE); // PAGE_ALIGN(mmap_min_addr);
	return hint;
}
		...
		unsigned long start, unsigned long end)
{
	...
	for (; it_addr < end; it_addr += HPAGE_PMD_SIZE) {
