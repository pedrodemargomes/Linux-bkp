...
{
	// pr_alert("free_pmd_range addr = %lx end = %lx", addr, end);
	pmd_t *pmd;
	unsigned long next;
	unsigned long start;
...
static inline int copy_pmd_range(struct mm_struct *dst_mm, struct mm_struct *src_mm,
		...
		unsigned long addr, unsigned long end)
{
	// pr_alert("copy_pmd_range addr = %lx end = %lx", addr, end);
...
{
	// pr_alert("copy_page_range start = %lx end = %lx", vma->vm_start, vma->vm_end);
...
again:
	...
	do {
		...
		if (pte_present(ptent)) {
			...
			// 	pr_alert("zap_pte_range page_to_pfn(page) = %ld PageTransCompound(page) = %d page_count(page) = %d total_mapcount(page) = %d", page_to_pfn(page), PageTransCompound(page), page_count(page), total_mapcount(page));
		...
		if (unlikely(!free_swap_and_cache(entry))) {
			pr_alert("print_bad_pte free_swap_and_cache");
	...
	do {
		...
		if (is_swap_pmd(*pmd) || pmd_trans_huge(*pmd) || pmd_devmap(*pmd)) {
			...
			if (next - addr != HPAGE_PMD_SIZE) {
				...
				// pr_alert("__split_huge_pmd addr = %lx PageTransCompound(page) = %d page_to_pfn(page) = %lx page_count(page) = %d total_mapcount(page) = %d", addr, PageTransCompound(page+1), page_to_pfn(page+1), page_count(page+1), total_mapcount(page+1));
			} else if (zap_huge_pmd(tlb, vma, pmd, addr)) {
				// pr_alert("next addr = %lx PageTransCompound(page) = %d page_to_pfn(page) = %lx page_count(page) = %d total_mapcount(page) = %d", addr, PageTransCompound(page), page_to_pfn(page), page_count(page), total_mapcount(page));
				// pr_alert("next addr = %lx PageTransCompound(page) = %d page_to_pfn(page) = %lx page_count(page) = %d total_mapcount(page) = %d", addr, PageTransCompound(page+1), page_to_pfn(page+1), page_count(page+1), total_mapcount(page+1));
		...
		if (pmd_none_or_trans_huge_or_clear_bad(pmd)) {
			// pr_alert("pmd_none_or_trans_huge_or_clear_bad");
			goto next;
		}
		...
		unsigned long end_addr)
{
	...
	// pr_alert("unmap_vmas start = %lx end = %lx", start_addr, end_addr);
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (is_zero_pfn(pte_pfn(vmf->orig_pte))) {
		...
		if (!cow_user_page(new_page, old_page, vmf)) {
			 ...
			 */
			pr_alert("wp_page_copy old_page = %ld", page_to_pfn(old_page));
			put_page(new_page);
			if (old_page)
				put_page(old_page);
			return 0;
	...
	if (PageAnon(vmf->page) && !PageKsm(vmf->page)) {
		...
		if (!trylock_page(vmf->page)) {
			...
			if (!pte_same(*vmf->pte, vmf->orig_pte)) {
				unlock_page(vmf->page);
				pte_unmap_unlock(vmf->pte, vmf->ptl);
				// pr_alert("do_wp_page page_to_pfn(page) = %ld", page_to_pfn(vmf->page));
				put_page(vmf->page);
				return 0;
			}
			// pr_alert("do_wp_page page_to_pfn(page) = %ld", page_to_pfn(vmf->page));
...
static vm_fault_t do_anonymous_page(struct vm_fault *vmf)
{
	...
	if (pte_alloc(vma->vm_mm, vmf->pmd, vmf->address)) {
		pr_alert("pte_alloc VM_FAULT_OOM");
		return VM_FAULT_OOM;
	}
	...
	if (PageTransCompound(page)) {
		pr_alert("memory.c after __SetPageUptodate PageTransCompound(page) == true");
	}
	// #endif
...
static vm_fault_t handle_pte_fault(struct vm_fault *vmf)
{
	...
	if (unlikely(pmd_none(*vmf->pmd))) {
		// pr_alert("unlikely(pmd_none(*vmf->pmd))");
		/*
		...
		if (pte_none(vmf->orig_pte)) {
			// pr_alert("pte_none(vmf->orig_pte)");
			pte_unmap(vmf->pte);
			vmf->pte = NULL;
		}
	}
		...
		unsigned long address, unsigned int flags)
{
	...
	if (rm_entry && rm_entry->mask != NULL && !(address < hstart || address + HPAGE_PMD_SIZE > hend) && bitmap_weight(rm_entry->mask, 512) > 64) {
		...
		for (i = 0; i < RESERV_NR; i++) {
			anon_vma = page_get_anon_vma(head+i);
			if (!anon_vma) {
				pr_alert("rm_alloc anon_vma = NULL page = %ld PageActive(page) = %d PageLRU(page) = %d page_count(page) = %d total_mapcount(page) = %d page->_mapcount = %d compound_mapcount(page) = %d PageTransCompound(page) = %d", page_to_pfn(head+i), PageActive(head+i), PageLRU(head+i), page_count(head+i), total_mapcount(head+i), atomic_read(&(head+i)->_mapcount), compound_mapcount(head+i), PageTransCompound(head+i));
				continue;
			}
			pr_alert("rm_alloc page = %ld PageActive(page) = %d PageLRU(page) = %d page_count(page) = %d total_mapcount(page) = %d page->_mapcount = %d compound_mapcount(page) = %d PageTransCompound(page) = %d", page_to_pfn(head+i), PageActive(head+i), PageLRU(head+i), page_count(head+i), total_mapcount(head+i), atomic_read(&(head+i)->_mapcount), compound_mapcount(head+i), PageTransCompound(head+i));
			anon_vma_lock_read(anon_vma);
			anon_vma_interval_tree_foreach(vmac, &anon_vma->rb_root, 0, ULONG_MAX) {
				_vma = vmac->vma;
				if (_vma && _vma->vm_mm)
					pr_alert("vma->vm_mm->owner->pid = %d", _vma->vm_mm->owner->pid);
			}
		...
		if (!retPrmtHugePage) {
			// if (is_stack_mapping(vma->vm_flags))
			// 	pr_alert("stack_mapping");
			// else
			// 	pr_alert("-");
			// if (vma->vm_flags & VM_SHARED)
			// 	pr_alert("VM_SHARED");
			...
			// pr_alert("current->pid = %d", task_pid_nr(current));
			return 0;
		} 
		// else {
		// 	pr_alert("promotion ERROR page_to_pfn(head) = %ld", page_to_pfn(head));
		// }
out:
		up_write(&mm->mmap_sem);
		down_read(&mm->mmap_sem);
